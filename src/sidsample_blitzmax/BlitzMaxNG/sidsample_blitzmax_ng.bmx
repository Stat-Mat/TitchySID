...
Include "titchysid_ng.bmx"

Extern "win32"
    Function EndDialog( hDlg:Byte Ptr, nResult:Byte Ptr )="BOOL EndDialog(HWND, BBINT)!"
    Function GetDlgItem:Byte Ptr( hDlg:Byte Ptr, nIDDlgItem )="HWND GetDlgItem(HWND, BBINT)!"
    Function ReleaseDC:Byte Ptr( hWnd:Byte Ptr, hDC:Byte Ptr )="INT ReleaseDC(HWND, HDC)!"
    Function OpenMutexA:Byte Ptr( dwDesiredAccess, bInheritHandle, lpName:Byte Ptr )="HANDLE OpenMutexA(BBINT, BBINT, LPCSTR)!"
    Function CreateMutexA:Byte Ptr( lpMutexAttributes:Byte Ptr, bInitialOwner, lpName:Byte Ptr )="HANDLE CreateMutexA(LPSECURITY_ATTRIBUTES, WINBOOL, LPCSTR)!"
    Function DialogBoxParamA:Byte Ptr( hInstance:Byte Ptr, lpTemplateName:Byte Ptr, hWndParent:Byte Ptr, lpDialogFunc:Byte Ptr, dwInitParam )="HANDLE DialogBoxParamA(HINSTANCE, LPCSTR, HWND, DLGPROC, LPARAM)!"
    Function SetDIBitsToDevice:Byte Ptr( hdc:Byte Ptr, XDest, YDest, dwWidth, dwHeight, XSrc, YSrc, uStartScan, cScanLines, lpvBits:Byte Ptr, lpbmi:Byte Ptr, fuColorUse )="INT SetDIBitsToDevice(HDC, int, int, DWORD, DWORD, int, int, UINT, UINT, CONST VOID*, CONST BITMAPINFO*, UINT)!"
    Function timeSetEvent:Byte Ptr( U_Delay, uResolution, lpTimeProc:Byte Ptr, dwUser:Byte Ptr, fuEvent )="MMRESULT timeSetEvent(UINT, UINT, LPTIMECALLBACK, DWORD_PTR, UINT)!"
    Function timeKillEvent:Byte Ptr( uTimerID:Byte Ptr )="MMRESULT timeKillEvent(UINT)!"
EndExtern
...
Global g_hInst:Byte Ptr
Global g_hWnd:Byte Ptr
...
Global specwin:Byte Ptr
Global mmtimer:Byte Ptr
...
Function DlgProc:Int(hWnd:Byte Ptr, uMsg, w_Param:WParam, l_Param:LParam) "Win32"
    Local pandr:String
    Local cstr:Byte Ptr
    Local a:Int   
    ...
    Select uMsg
        Case WM_INITDIALOG
            ...
            If OpenMutexA(MUTEX_ALL_ACCESS, False, cstr) = 0
                ...
                SendMessageA(hWnd, WM_SETICON, 0, LParam(LoadIconA(g_hInst, Byte Ptr(IDI_ICON))))
                ...
                BitmapInfo256Buffer = MemAlloc(Size_T(SizeOf(BITMAPINFOHEADER) + (SizeOf(RGBQUAD) * 256)))

                MemCopy(BitmapInfo256Buffer, Varptr(bmiHeader.biSize), Size_T(SizeOf(BITMAPINFOHEADER)))
                ...
                Case IDC_NEXT
                    If Not g_paused        
                        ...
                        If g_subsong < sid_props.num_songs
                            ...
                            SIDChangeSong(Byte(g_subsong - 1))
                            UpdateSIDInfo()
                        End If
                    End If
        ...
        Case WM_LBUTTONDOWN
            SendMessageA(hWnd, WM_NCLBUTTONDOWN, HTCAPTION, l_Param)
